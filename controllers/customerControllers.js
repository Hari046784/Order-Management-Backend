...
exports.updateCustomers = (req, res) => {
    try{
        Customers.findByIdAndUpdate({_id: req.params.id}, {$set: req.body}, (err, data) => {
            if (err) {
                return res.status(400).send({
                    message: "Error while updating an existing customer"
                });
            }
            res.status(201).send({
                id: data._id,
...
exports.deleteCustomers = (req, res) => {
    try{
        Customers.deleteOne({_id:req.params.id}, (err, data) => {
            if(err) {
                return res.status(400).send({
                    message: "Error while deleting an customer"
                });
            }
            res.status(200).send({
...
exports.dashboardCustomersDetails = async (req, res) => {
    try{
        const value = await Customers.find();
        res.status(200).send({
            totalCustomers: value.length
        });
    } catch(error) {
        res.status(500).send({
            message: "Internal Server Error"
